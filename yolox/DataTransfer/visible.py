...
def draw_box(img, objects, draw=True):
    ...
    for object in objects:
        ...
        if draw:
            ...
            cv2.putText(img, 'gt', (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 4)
    return img
...
def show_image(image_path, anno_path, show=False, plot_image=False):
    ...
    anno_file_list = [anno_path+"/"+file for file in os.listdir(anno_path) if file.endswith(".xml")]
    ...
    for xml_file in tqdm(anno_file_list):
        ...
        if 'object' in xml_info_dict['annotation'].keys():
            img = draw_box(img, xml_info_dict['annotation']['object'], draw=True)
            cv2.imwrite("../mydata/test/images_gt/"+filename, img, [int(cv2.IMWRITE_JPEG_QUALITY), 95])
        # if show:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ip', '--image-path', type=str, default='../mydata/test/images', help='image path')
    parser.add_argument('-ap', '--anno-path', type=str, default='../mydata/test/xmls', help='annotation path')
    ...
    if len(sys.argv) > 1:
        ...
        image_path = '../mydata/test/images'
        anno_path = '../mydata/test/xmls'
